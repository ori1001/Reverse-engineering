...
// Global variables
CHAR OrigOpcode[6] = "\x83\xC4\x80\x8D\x45"; // Restores overriden bytes from hooked function
...
void _stdcall replace_bytes()
{
    HMODULE h = GetModuleHandle(L"codes.exe");
    FUNC_PTR replace_loc = (char*)((long)h + (long)0x1486);
    DWORD lpProtect = 0;
    char newOpcode[4] = "\x8B\x45\x10";
    VirtualProtect((char*)replace_loc, 0x3, PAGE_EXECUTE_READWRITE, &lpProtect);
    memcpy((char*)replace_loc, &newOpcode, 0x3);
    VirtualProtect((char*)replace_loc, 0x3, PAGE_EXECUTE_READ, &lpProtect);
}
...
void setHook() {
    ...
    original_func_address = (char*)((long)h + (long)0x1416); //TODO: test this
